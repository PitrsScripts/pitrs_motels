...
local db = setmetatable({},{
	__call = function(self)
		...
		self.insert = function(...)
			...
			return MySQL.insert.await(str:format('pitrs_motels','motel','hour_rate','revenue','employees','rooms','owned'),{...})
		end

		self.update = function(column, where, string, data)
			local str = 'UPDATE %s SET %s WHERE %s = ?'
			return MySQL.update(str:format('pitrs_motels',column,where),{data,string})
		end

		self.updateall = function(pattern, where, string, ...)
			local str = 'UPDATE %s SET % WHERE %s = ?'
			local str = 'UPDATE pitrs_motels SET '..pattern..' WHERE '..where..' = ?'
			local data = {...}
		...
		self.query = function(column, where, string)
			...
			return MySQL.query.await(str:format(column,'pitrs_motels',where),{string})
		end

		self.fetchAll = function()
			local str = 'SELECT * FROM pitrs_motels'
...
Citizen.CreateThreadNow(function()
	local success, result = pcall(MySQL.scalar.await, 'SELECT 1 FROM pitrs_motels')
	if not success then
		MySQL.query.await([[CREATE TABLE `pitrs_motels` (
	...
	for k,v in pairs(config.motels) do
		local query = MySQL.query.await('SELECT rooms FROM pitrs_motels WHERE `motel` = ?',{v.motel})
		if not query[1] then
			local doors = {}
